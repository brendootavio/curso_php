...
try {
    ...
    $pdo->query("INSERT INTO users (first_name, last_name, email, document) VALUES ('Brendo', 'Oliveira', 'brendo.oliveira@codeandsoul.com', '721982')
    ");

    $userId = $pdo->lastInsertId();

    $pdo->query("INSERT INTO users_address (user_id, street, number, complement)
    VALUES ('{$userId}', 'Rua das nuvens', '748', 'lado do mercado')
    ");
    if($pdo->commit()){
        echo "<p class='trigger accept'>Usuario cadastrado com succeso</p>";
    }else{
        echo "<p class='trigger error'>Erro ao cadastrar usuario</p>";
    }
